					...
					<div class="row">
						<div class="col-sm-12">
							<div class="form-group form-group-default">
								...
								<input id="addName" type="text" maxlength="{{ limit("user_type.max")}}" class="form-control" placeholder="Event Type" name="name">
							</div>
						</div>
...
@push("js")
	...
	<script type="text/javascript">
		var delete_url = "{{ route('private.eventtype.destroy') }}";
		var table;
		...
		$(document).ready(function() {
	    	...
	    	table = $('#record-table').dataTable({
	            ...
	            "serverSide": true,
	            "ajax": {
	                "url": "{{ route("private.eventtype.list") }}",
	                "type": "POST",
	                data: function (d) {     
	            ...
	            "columns": [
	                    ...
	                    {   "mRender": function ( data, type, row ) 
	                        {
	                        	var btn = `<div class="form-button-action"> 
		                        				<a href="" data-toggle="tooltip" title="" class="btn btn-link btn-primary btn-lg set-permission d-none" data-id="${row.id}" data-original-title="Set Permission" target="_blank"> <i class="fas fa-shield-alt"></i> </a> 
	               ...
	               createdRow: function( row, data, dataIndex ) {
	                              ...
	                              $(row).find( '.status_edit' ).editable({
	                                    url: "{{ route('private.eventtype.status') }}",
	        ...
	        $("#addForm").validate({
	            ...
	            submitHandler: function(form) {
	                ...
	                $.ajax({
	                    type: "POST",
	                    url: "{{ route('private.eventtype.create') }}",
	                    data: data,
	                    dataType: "json",
	        ...
	        $("#editForm").validate({
	            ...
	            submitHandler: function(form) {
	                ...
	                $.ajax({
	                    type: "POST",
	                    url: "{{ route('private.eventtype.update') }}",
	                    data: data,
	                    dataType: "json",
	                    success: function(data) {
	                        loadButton("#editRowButton");
            ...
            $("table").on("click", ".edit-data", function(){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('private.eventtype.edit')}}",
                    data: {data:dataId},
                    dataType: "json",
