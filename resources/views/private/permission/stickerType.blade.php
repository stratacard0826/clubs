					...
					<div class="row">
						<div class="col-sm-12">
							<div class="form-group form-group-default">
								...
								<input id="addName" type="text" maxlength="{{ limit("user_type.max")}}" class="form-control" placeholder="Sticker Type" name="name">
							</div>
						</div>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-sm-12">
							<div class="form-group form-group-default">
								<label>Sticker Type</label>
								<input id="editName" type="text" maxlength="{{ limit("user_type.max")}}" class="form-control" placeholder="Sticker Type" name="name">
							</div>
						</div>
					</div>
				</div>
...
@push("js")
	...
	<script type="text/javascript">
		var delete_url = "{{ route('private.stickertype.destroy') }}";
		var table;
		...
		$(document).ready(function() {
	    	...
	    	table = $('#record-table').dataTable({
	            ...
	            "serverSide": true,
	            "ajax": {
	                "url": "{{ route("private.stickertype.list") }}",
	                "type": "POST",
	                data: function (d) {     
	            ...
	            "columns": [
	               ...
	               createdRow: function( row, data, dataIndex ) {
	                              ...
	                              $(row).find( '.status_edit' ).editable({
	                                    url: "{{ route('private.stickertype.status') }}",
	        ...
	        $("#addForm").validate({
	            ...
	            submitHandler: function(form) {
	                ...
	                $.ajax({
	                    type: "POST",
	                    url: "{{ route('private.stickertype.create') }}",
	                    data: data,
	                    dataType: "json",
	        ...
	        $("#editForm").validate({
	            ...
	            submitHandler: function(form) {
	                ...
	                $.ajax({
	                    type: "POST",
	                    url: "{{ route('private.stickertype.update') }}",
	                    data: data,
	                    dataType: "json",
	                    success: function(data) {
	                        loadButton("#editRowButton");
            ...
            $("table").on("click", ".edit-data", function(){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('private.stickertype.edit')}}",
                    data: {data:dataId},
                    dataType: "json",
