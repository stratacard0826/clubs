							...
							<div class="form-group pt-0">
								...
								<select class="select-filter form-control" data-placeholder="Select a Event" id="event-type-filter">
	                              	...
	                              	@foreach($eventTypes as $key=>$eventType)
	                              		@if($eventType->parent_id==0)
	                              		<optgroup label="{{$eventType->name}}">
	                              			@foreach($eventTypes as $key=>$category)
		                              			@if($eventType->id == $category->parent_id)
								...
								<div class="row">
									...
									<div class="col-sm-12">
										<div class="form-group form-group-default">
											...
											<input id="addOrg" type="text" name="organisation" maxlength="{{ limit("event_organisation.max")}}" class="form-control" placeholder="Organisation Name">
										</div>
									</div>
									<div class="col-md-6">
										<div class="form-group form-group-default">
											<label>Event Start Date</label>
											<input id="addStartDate" type="text" name="event_start_date" class="form-control date-time-filter" placeholder="Select a start date">
										</div>
									</div>
									<div class="col-md-6">
										<div class="form-group form-group-default">
											<label>Event End Date</label>
											<input id="addEndDate" type="text" name="event_end_date" class="form-control date-time-filter" placeholder="Select a end date">
										</div>
									</div>
									<div class="col-md-12">
										<div class="form-group form-group-default">
		...
		<div class="modal-content">
			...
			<form id="editForm">
				<div class="modal-body">
						...
						<input name="data" type="hidden"  class="form-control" id="editData">
						<div class="row">
							<div class="col-sm-12">
								<div class="row">
									<div class="col-sm-12">
									...
									<div class="col-sm-12">
										<div class="form-group form-group-default">
											<label>Organisation Name</label>
											<input id="editOrg" type="text" name="organisation" maxlength="{{ limit("event_organisation.max")}}" class="form-control" placeholder="Organisation Name">
										</div>
									</div>
									<div class="col-md-6">
										<div class="form-group form-group-default">
											<label>Event Start Date</label>
											<input id="editStartDate" type="text" name="event_start_date" class="form-control date-time-filter" placeholder="Select a start date">
										</div>
									</div>
									<div class="col-md-6">
										<div class="form-group form-group-default">
											<label>Event Date</label>
											<input id="editEndDate" type="text" name="event_end_date" class="form-control date-time-filter" placeholder="Select a end date">
										</div>
									</div>
									<div class="col-md-12">
										<div class="form-group form-group-default">
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
	    	...
	    	table = $('#record-table').dataTable({
	            ...
	            "serverSide": true,
	            "ajax": {
	                "url": "{{ route("private.events.list") }}",
	                "type": "POST",
	                data: function (d) { 
	                	d.event_type = $("#event-type-filter").val()     
	                	d.status = $("#status-filter").val() 
	                	d.start_date = startDate1
						d.end_date = endDate1      
	                },
	            },
			...
			addValidator = $("#addForm").validate({
			    rules: {
					...
					organisation:{
			                    required: true,
			                    minlength	: {{ limit("event_organisation.min") }},
			                    maxlength	: {{ limit("event_organisation.max") }}
			                }
			    },
			...
			/*$('.date-time-filter').on('cancel.daterangepicker', function(ev, picker) {
				let dateId = picker.element[0].id;
				if(dateId == "addStartDate"){
					startDate = "";
					startTime = "";
				}else if(dateId == "addEndDate"){
					endDate =  "";
					endTime = "";
	        ...
	        $.ajax({
                ...
                success: function(data) {
                	if(data.success == 1){
                        ...
                        $('#editOrg').val(record.organisation);
                        ...
                        var editDatePickerOption1 = datePickerOption;
                        var editDatePickerOption2 = datePickerOption;
                        editDatePickerOption1.startDate = moment(record.start_date);
                        editDatePickerOption2.startDate = moment(record.end_date);
                        if(moment().diff(datePickerOption.startDate) > 0){
                        	lg("hi");
	                        editDatePickerOption1.minDate = moment(record.start_date);
	                        editDatePickerOption2.minDate = moment(record.start_date);
                        }
                        startDate = moment(record.start_date).format('YYYY-MM-DD HH:mm:ss');
						startTime = moment(record.start_date).format('HH:mm:ss');
						endDate =  moment(record.end_date).format('YYYY-MM-DD HH:mm:ss');
						endTime = moment(record.end_date).format('HH:mm:ss');
						$('#editStartDate').val(moment(record.start_date).format('{{ config("site.date_time_format.front") }}'));
						$('#editEndDate').val(moment(record.end_date).format('{{ config("site.date_time_format.front") }}'));
                        $("#editStartDate.date-time-filter").daterangepicker(editDatePickerOption2, datePickerEvent);
                        $("#editEndDate.date-time-filter").daterangepicker(editDatePickerOption2, datePickerEvent);
                        initalizeEvent();
	    ...
	    function initalizeEvent(){
	    	$('.date-time-filter, #editStartDate.date-time-filter, #editEndDate.date-time-filter').on('apply.daterangepicker', function(ev, picker) {
				let dateId = picker.element[0].id;
				if(dateId == "addStartDate" || dateId == "editStartDate"){
					startDate = picker.startDate.format('YYYY-MM-DD HH:mm:ss');
					startTime = picker.startDate.format('HH:mm:ss');
				}else if(dateId == "addEndDate" || dateId == "editEndDate"){
					endDate =  picker.startDate.format('YYYY-MM-DD HH:mm:ss');
					endTime = picker.startDate.format('HH:mm:ss');

				}
				$('#'+dateId).val(picker.startDate.format('{{ config("site.date_time_format.front") }}'));
			  console.log(startDate);
			  console.log(startTime);
			  console.log(endDate);
			  console.log(endTime);
